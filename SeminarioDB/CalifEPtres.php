...
$p21 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 51 AND Correcto = 1");
while($row21 = mysqli_fetch_assoc($p21)){
    $r21 = $_POST['resP21'] == $row21['Respuesta'] ? $count++ : "";
}
$p22 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 52 AND Correcto = 1");
while($row22 = mysqli_fetch_assoc($p22)){
    $r22 = $_POST['resP22'] == $row22['Respuesta'] ? $count++ : "";
}
$p23 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 53 AND Correcto = 1");
while($row23 = mysqli_fetch_assoc($p23)){
    $r23 = $_POST['resP23'] == $row23['Respuesta'] ? $count++ : "";
}
$p24 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 54 AND Correcto = 1");
while($row24 = mysqli_fetch_assoc($p24)){
    $r24 = $_POST['resP24'] == $row24['Respuesta'] ? $count++ : "";
}
$p25 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 55 AND Correcto = 1");
while($row25 = mysqli_fetch_assoc($p25)){
    $r25 = $_POST['resP25'] == $row25['Respuesta'] ? $count++ : "";
}
$p26 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 56 AND Correcto = 1");
while($row26 = mysqli_fetch_assoc($p26)){
    $r26 = $_POST['resP26'] == $row26['Respuesta'] ? $count++ : "";
}
$p27 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 57 AND Correcto = 1");
while($row27 = mysqli_fetch_assoc($p27)){
    $r27 = $_POST['resP27'] == $row27['Respuesta'] ? $count++ : "";
}
$p28 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 58 AND Correcto = 1");
while($row28 = mysqli_fetch_assoc($p28)){
    $r28 = $_POST['resP28'] == $row28['Respuesta'] ? $count++ : "";
}
$p29 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 59 AND Correcto = 1");
while($row29 = mysqli_fetch_assoc($p29)){
    $r29 = $_POST['resP29'] == $row29['Respuesta'] ? $count++ : "";
}
$p30 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 3 AND idPregunta = 60 AND Correcto = 1");
...
while($rowI = mysqli_fetch_assoc($intentos)){
    if($rowI['IntentosTres'] != NULL){
        ...
        $qIntent = mysqli_query($conexion, "UPDATE Examenes SET CaliUnidadTres = $count, IntentosTres = IntentosTres + 1 WHERE Username = '".$_SESSION['usuario']."' AND idCUrso = 2");
