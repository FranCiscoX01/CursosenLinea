...
$p1 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 31 AND Correcto = 1");
while($row1 = mysqli_fetch_assoc($p1)){
    $r1 = $_POST['resP1'] == $row1['Respuesta'] ? $count++ : "";
}
$p2 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 32 AND Correcto = 1");
while($row2 = mysqli_fetch_assoc($p2)){
    $r2 = $_POST['resP2'] == $row2['Respuesta'] ? $count++ : "";
}
$p3 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 33 AND Correcto = 1");
while($row3 = mysqli_fetch_assoc($p3)){
    $r3 = $_POST['resP3'] == $row3['Respuesta'] ? $count++ : "";
}
$p4 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 34 AND Correcto = 1");
while($row4 = mysqli_fetch_assoc($p4)){
    $r4 = $_POST['resP4'] == $row4['Respuesta'] ? $count++ : "";
}
$p5 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 35 AND Correcto = 1");
while($row5 = mysqli_fetch_assoc($p5)){
    $r5 = $_POST['resP5'] == $row5['Respuesta'] ? $count++ : "";
}
$p6 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 36 AND Correcto = 1");
while($row6 = mysqli_fetch_assoc($p6)){
    $r6 = $_POST['resP6'] == $row6['Respuesta'] ? $count++ : "";
}
$p7 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 37 AND Correcto = 1");
while($row7 = mysqli_fetch_assoc($p7)){
    $r7 = $_POST['resP7'] == $row7['Respuesta'] ? $count++ : "";
}
$p8 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 38 AND Correcto = 1");
while($row8 = mysqli_fetch_assoc($p8)){
    $r8 = $_POST['resP8'] == $row8['Respuesta'] ? $count++ : "";
}
$p9 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 39 AND Correcto = 1");
while($row9 = mysqli_fetch_assoc($p9)){
    $r9 = $_POST['resP9'] == $row9['Respuesta'] ? $count++ : "";
}
$p10 = mysqli_query($conexion, "SELECT Respuesta FROM Respuestas WHERE idCurso = 2 AND Unidad = 1 AND idPregunta = 40 AND Correcto = 1");
...
while($rowI = mysqli_fetch_assoc($intentos)){
    if($rowI['IntentosUno'] != NULL){
        ...
        $qIntent = mysqli_query($conexion, "UPDATE Examenes SET CaliUnidadUno = $count, IntentosUno = IntentosUno + 1 WHERE Username = '".$_SESSION['usuario']."' AND idCUrso = 2");
        ...
        $qGeneral = mysqli_query($conexion, "UPDATE Examenes SET CaliUnidadUno = $count, IntentosUno = 1 WHERE Username = '".$_SESSION['usuario']."' AND idCUrso = 2");
    }
}
